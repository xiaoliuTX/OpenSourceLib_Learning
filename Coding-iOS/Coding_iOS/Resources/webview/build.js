...
var generateHtml = function(source_name, target_name, with_tpl_content){
    ...
    inline(path.join(__dirname, source_name), function (html) {
        if (with_tpl_content) {
            //var tpl = fs.readFileSync(path.join(__dirname, 'app/template/template.tpl'), {encoding: 'utf-8'})
            var tpl = fs.readFileSync(path.join(__dirname, 'app/template/markdown.html'), {encoding: 'utf-8'})
            //var tpl = fs.readFileSync(path.join(__dirname, 'app/template/quote.json'), {encoding: 'utf-8'})
